...
int main() {
  ...
  /* Initialize a range. */
  const int64_t range[] = { 3, 3, 3, 3 };

  /* Subset over attribute "a1" and the coordinates. */
  const char* attributes[] = { TILEDB_COORDS, "a1" };
  ...
  char buffer_var_a1[26]; 
  void* buffers[] = { buffer_coords, buffer_a1, buffer_var_a1 };
  size_t buffer_sizes[] = { 
  ...
  int result_num = buffer_sizes[0] / sizeof(int64_t) / 2;
  for(int i=0; i<result_num; ++i) { 
    std::cout << "(" << buffer_coords[2*i] << ", " << buffer_coords[2*i+1]  
              << "): " << buffer_var_a1 + buffer_a1[i] << "\n";
  }
