...
def parse_history(pr_message):
  ...
  if len(h_types) == 0:
    print_and_die("Unable to locate \"" + type_tag + "\" in PR message:\n" + pr_message)

  if len(h_descriptions) == 0:
    print_and_die("Unable to locate \"" + description_tag + "\" in PR message:\n" + pr_message)
...
def format_history_line(pr_number, h_description):
  return "* " + h_description + " [#" + pr_number + "](https://github.com/TileDB-Inc/TileDB/pull/" + pr_number + ")"
