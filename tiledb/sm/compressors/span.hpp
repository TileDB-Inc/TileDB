...
#define span_REQUIRES_T(VA)   \
  , typename = typename std:: \
        enable_if<(VA), nonstd::span_lite::detail::enabler>::type
#else
...
span_noreturn inline void throw_out_of_range(size_t idx, size_t size) {
  ...
  sprintf(
      buffer, fmt, static_cast<long long>(idx), static_cast<long long>(size));
...
class span {
 public:
  ...
  template <typename It span_REQUIRES_T(
      (std::is_convertible<decltype(*std::declval<It&>()), element_type&>::
           value))>
  ...
  template <
      ...
      extent_type OtherExtent span_REQUIRES_T(
          ((Extent == dynamic_extent || OtherExtent == dynamic_extent ||
            Extent == OtherExtent) &&
           std::is_convertible<OtherElementType (*)[], element_type (*)[]>::
               value))>
  span_constexpr_exp span(span<OtherElementType, OtherExtent> const& other)
      span_noexcept : data_(other.data()),
                      size_(other.size()) {
    span_EXPECTS(
        OtherExtent == dynamic_extent || other.size() == to_size(OtherExtent));
  }
  ...
  span_constexpr_exp span<element_type, dynamic_extent> last(
      size_type count) const {
    span_EXPECTS(detail::is_positive(count) && count <= size());
  ...
  span_constexpr_exp span<element_type, dynamic_extent> subspan(
    ...
    return span<element_type, dynamic_extent>(
        data() + offset,
        count == static_cast<size_type>(dynamic_extent) ? size() - offset :
                                                          count);
  }
...
#if span_HAVE(ITERATOR_CTOR)
  static inline span_constexpr pointer to_address(std::nullptr_t)
      span_noexcept {
    return nullptr;
  }
...
span(It, EndOrSize)
    ->span<typename std11::remove_reference<
...
inline span_constexpr bool same(span<T1, E1> const& l, span<T2, E2> const& r)
    span_noexcept {
...
// Avoid MSVC 14.1 (1910), VS 2017: warning C4307: '*': integral constant
// overflow:
...
template <class T, std::size_t N>
inline span_constexpr span<T, static_cast<extent_t>(N)> make_span(T (&arr)[N])
    span_noexcept {
...
template <class T, std::size_t N>
inline span_constexpr span<T, static_cast<extent_t>(N)> make_span(
    std::array<T, N>& arr) span_noexcept {
...
inline span_constexpr span<T> make_span(std::vector<T, Allocator>& cont)
    span_noexcept {
  return span<T>(with_container, cont);
}

template <class T, class Allocator>
inline span_constexpr span<const T> make_span(
    std::vector<T, Allocator> const& cont) span_noexcept {
...
template <class T, extent_t Extent>
span_constexpr span<T> subspan(
    span<T, Extent> spn, size_t offset, extent_t count = dynamic_extent) {
...
template <class T>
span_constexpr auto subspan(
    T& t, size_t offset, extent_t count = dynamic_extent)
...
inline span_constexpr auto byte_span(T& t) span_noexcept
  ...
  return span<std17::byte, span_sizeof(t)>(
      reinterpret_cast<std17::byte*>(&t), span_sizeof(T));
}
...
template <size_t I, typename ElementType, nonstd::span_lite::extent_t Extent>
span_constexpr ElementType& get(nonstd::span<ElementType, Extent>& spn)
    span_noexcept {
#if span_HAVE(STATIC_ASSERT)
  static_assert(
      Extent != nonstd::dynamic_extent && I < Extent,
...
template <size_t I, typename ElementType, nonstd::span_lite::extent_t Extent>
span_constexpr ElementType const& get(
    nonstd::span<ElementType, Extent> const& spn) span_noexcept {
#if span_HAVE(STATIC_ASSERT)
  static_assert(
      Extent != nonstd::dynamic_extent && I < Extent,
      "get<>(span): dynamic extent or index out of range");
