...
class condition_variable_any2 {
 ...
 public:
  ...
  void wait(Lockable& lock) {
    ...
    std::unique_lock<std::mutex> second_internal_lock(
        std::move(first_internal_lock));
    local_internals->cv.wait(second_internal_lock);
  }

  template <class Lockable, class Predicate>
  void wait(Lockable& lock, Predicate pred) {
    // have to manually implement the loop so that the user-provided lock is
    // reacquired before calling pred(). (otherwise the test_cvrace_pred test
    // case fails)
    auto local_internals = internals;
    while (!pred()) {
      std::unique_lock<std::mutex> first_internal_lock(local_internals->m);
  ...
  cv_status wait_until(
      Lockable& lock, const chrono::time_point<Clock, Duration>& abs_time) {
    auto local_internals = internals;
    std::unique_lock<std::mutex> first_internal_lock(local_internals->m);
    unlock_guard<Lockable> unlocker(lock);
    std::unique_lock<std::mutex> second_internal_lock(
  ...
  cv_status wait_for(
      Lockable& lock, const chrono::duration<Rep, Period>& rel_time) {
  ...
  bool wait_for(
      ...
      Predicate pred) {
    return wait_until(
        lock, std::chrono::steady_clock::now() + rel_time, std::move(pred));
  }
  ...
  template <class Lockable, class Predicate>
  bool wait(Lockable& lock, stop_token stoken, Predicate pred);
 ...
 private:
  ...
  //   threads shall have been notified;
  //             they may subsequently block on the lock specified in the wait.
  //             This relaxes the usual rules, which would have required all
  //             wait calls to happen before destruction. Only the notification
  //             to unblock the wait needs to happen before destruction. The
  //             user should take care to ensure that no threads wait on *this
  //             once the destructor has been started, especially when the
  //             waiting threads are calling the wait functions in a loop or
  //             using the overloads of wait, wait_for, or wait_until that take
  //             a predicate.  ]
  // That big long note means ~condition_variable_any() can execute before a
  // signaled thread returns from a wait. If this happens with
  // condition_variable_any2, that waiting thread will attempt to lock the
  // destructed mutex mut. To fix this, there must be shared ownership of the
  // data member mut between the condition_variable_any object and the member
  // functions wait (wait_for, etc.). (libc++'s implementation gets this right:
  ...
  //  entry so that if *this destructs out from under the thread executing the
  //  wait function, the mutex stays alive until the wait function returns.)
};
...
// - false otherwise (i.e. on interrupt)
template <class Lockable, class Predicate>
inline bool condition_variable_any2::wait(
    Lockable& lock, stop_token stoken, Predicate pred) {
  if (stoken.stop_requested()) {
    return pred();
  }
  auto local_internals = internals;
  stop_callback cb(
      stoken, [&local_internals] { local_internals->notify_all(); });
  while (!pred()) {
    std::unique_lock<std::mutex> first_internal_lock(local_internals->m);
    if (stoken.stop_requested()) {
...
// - false otherwise (i.e. on timeout or interrupt)
template <class Lockable, class Clock, class Duration, class Predicate>
inline bool condition_variable_any2::wait_until(
    Lockable& lock,
    stop_token stoken,
    const chrono::time_point<Clock, Duration>& abs_time,
    Predicate pred) {
  ...
  // have to manually implement the loop so that the user-provided lock is
  // reacquired before calling pred(). (otherwise the test_cvrace_pred test case
  // fails)
  auto local_internals = internals;
  stop_callback cb(
      stoken, [&local_internals] { local_internals->notify_all(); });
  while (!pred()) {
    bool shouldStop;
    {
      std::unique_lock<std::mutex> first_internal_lock(local_internals->m);
      if (stoken.stop_requested()) {
        // pred() has already evaluated to 'false' since we last acquired
        // 'lock'.
        return false;
      }
      unlock_guard<Lockable> unlocker(lock);
      ...
      const auto status =
          local_internals->cv.wait_until(second_internal_lock, abs_time);
      shouldStop =
          (status == std::cv_status::timeout) || stoken.stop_requested();
    }
    if (shouldStop) {
      return pred();
    }
...
// - false otherwise (i.e. on timeout or interrupt)
template <class Lockable, class Rep, class Period, class Predicate>
inline bool condition_variable_any2::wait_for(
    Lockable& lock,
    stop_token stoken,
    const chrono::duration<Rep, Period>& rel_time,
    Predicate pred) {
  auto abs_time = std::chrono::steady_clock::now() + rel_time;
  return wait_until(lock, std::move(stoken), abs_time, std::move(pred));
}
