...
* Any number of [data files](#data-file). For each fixed-sized attribute `attribute` (or dimension `dimension`), there is a single data file `a0.tdb` (`d0.tdb`) containing the values along this attribute (dimension). For every var-sized attribute `a1` (or dimensions `d1`), there are two data files; `a1_var.tdb` (`d1_var.tdb`) containing the var-sized values of the attribute (dimension) and `a1.tdb` (`d1.tdb`) containing the starting offsets of each value in `a1_var.tdb` (`d1_var.rdb`). Both fixed-sized and var-sized attributes can be nullable. A nullable attribute, `attribute_nullable`, will have an additional file `a2_validity.tdb` that contains its validity vector.
